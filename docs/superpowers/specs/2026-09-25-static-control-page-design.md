...
| §01a 累计增长图 | 17 个版本徽章 + tooltip（年份/版本/新增/累计/增长率/叙述/来源）；3 个范围按钮；版本对比下拉 | ① 全范围主图，每个徽章旁标编号；② 下方「2015 → 至今」放大小图（覆盖 2015/2020 两档范围按钮）；③ 17 行表：编号、年份、版本、新增、累计、增长率、代表表情、叙述、来源。版本对比是表中数据的组合运算，不单独枚举 |
| §01b 分类 treemap | 时间滑块切换 17 帧 | 最新一帧大图（= 交互版默认视图）+ 17 张小 treemap 按时间网格平铺，每张标版本与年份，沿用同一套 3 档标签规则 |
| §01c 变体 Sankey | 22 条流，hover/focus/pin 才出现表情珠与 tooltip | 22 条流**全部**显示表情珠（沿用 `sampleSankeyCurve` 与珠数/珠径公式）；数值标在流或节点旁；原 `sr-only` `<details>` 汇总改为可见表格 |
...
1. **内容对等**（`tests/static/parity.test.tsx`）：渲染静态页各组件，断言 17 个版本行、17 张 treemap、22 条 Sankey 流、41 个地图标注、5 个流程步骤、6 张标准卡、4 个案例全部出现，且数值与数据 JSON 一致。
